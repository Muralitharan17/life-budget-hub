...
export function useBudgetData() {
  ...
  const saveBudgetConfig = async (config: Omit<BudgetConfig, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => {
  ...
  const saveInvestmentPortfolio = async (portfolio: Omit<InvestmentPortfolio, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => {
    if (!user) return { error: 'User not authenticated' }
